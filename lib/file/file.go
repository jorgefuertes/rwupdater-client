...
// Abbr the path and name
func (f *File) Abbr() string {
	name := f.Path + "/" + f.Name
	if len(name) <= MaxAbbr {
		return name
	}

	return "..." + name[len(name)-MaxAbbr-3:]
}
...
// GetLocalCatalog
func GetLocalCatalog(root string, path string, curRec int) (*Catalog, error) {
	var cat Catalog
	...
	for _, entry := range dir {
		...
		if entry.IsDir() {
			subdir, err := GetLocalCatalog(root, path+"/"+entry.Name(), curRec+1)
			if err != nil {
				return &cat, err
			}
			if len(*subdir) > 0 {
